/// Elliptic curve group based on binary field, say E(\bar{K}): x^2 + xy = x^3 + a_2 * x^2 + a_6, where \bar{K} is the field on which EC group elements is defined
...
// Binary curve with short Weierstrass equation: y^2 + xy = x^3 + a_2 * x^2 + a_6, in terms of K-233 curve, a_2 = 0, a_6 = 1
// We use Lopez-Dahab projective coordinates, the Weierstrass equation is: Y^2 + X * Y * Z = X^3 * Z + a_2 * X^2 * Z^2 + a_6 * Z^4
